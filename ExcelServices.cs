    ...
    public static class ExcelServices
    {
        ...
        public static void ApplyFormulaAdjustFeeToExcel(string targetScope, string groupName, System.Collections.Generic.List<Controllers.FormulaItemModel>? items = null)
        {
            try
            {
                ...
                {
                    // 寻觅光标落点所在的箱柜 (支持顶部汇总行、明细大标题 detRow-3 到总计行 tolsumRow 完整区间)
                    ...
                    foreach (var c in validCabinets)
                    {
                        ...
                        int tolsumRow = c.Value.Tolsum != null ? Convert.ToInt32(c.Value.Tolsum.Row) : detRow + 27;
                        // 明细块起始大标题行号 (detRow - 3)
                        int detBlockStartRow = detRow - 3;

                        // 判断光标是否在当前箱柜的汇总行或明细块区域内
                        if (activeRow == sumRow || (activeRow >= detBlockStartRow && activeRow <= tolsumRow))
                        {
                            matched = c;
                            break;
                        }
                ...
                try
                {
                    ...
                    foreach (var cabPair in targetCabinetsDesc)
                    {
                        ...
                        // ── 步骤一：调用 Tool.AlignRowRangeCount 对齐计费区域行数 ──
                        int rowDiff = Tool.AlignRowRangeCount(activeSheet, oldSubsumRow, oldFeeRowCount, newFeeRowCount);
                        // 行数发生变动后，更新最新的总计行物理行号
                        tolsumRow += rowDiff;
                        ...
                        {
                            // 记录定义名称更新异常
                            LogHelper.WriteLog($"[调试] 箱柜[{k}]刷新定义名称异常: {exName.Message}");
                        }

                        // ── 步骤五：更新汇总行 G/H 列公式联动 ──
                        // G 列（单价合计）= 明细总计行 H 列（销售总价总计）(规则 6)
                        activeSheet.Cells[sumRow, 7].Formula = $"=H{tolsumRow}";
                        // H 列（总价）= 数量(F) × 单价(G)
        ...
        public static void CreateNewCabinetFromSelection()
        {
            try
            {
                ...
                try
                {
                    ...
                    // 声明复制源箱柜结构与目标插入行
                    Models.CabinetAnchorModel srcCabinet = null;
                    // 声明动态计算的复制源起始行与终止行
                    ...
                    else
                    {
                        ...
                        // 2. 复制源选择工作表中的“最后一个箱柜”
                        srcCabinet = validCabinets.Count > 0 ? validCabinets.Last().Value : null;
