...
- **已完成架构与功能**：
  ...
  5. **箱柜下拉菜单与“新建箱柜”功能实施**：
     ...
     - 在 [ExcelServices.cs](file:///c:/Users/ADMIN/.gemini/antigravity/scratch/ExcelAddInCTtools/ExcelServices.cs) 中重构并实现了 `CreateNewCabinetFromSelection()`：
       ...
       4. **定义名称锚点超链接**：自动在顶部与底部单元格注册工作簿定义名称（`Cab_Sum_K` 与 `Cab_Det_K`），并创建基于定义名称的超链接。修复了 `SubAddress` 误加 `'工作表名'!` 前缀导致 Excel 将工作簿作用域（Workbook Scope）名称误认为工作表作用域名称抛出“无效引用”的问题；同时通过监听 `SheetFollowHyperlink` 事件，读取配置文件中 `ScrollRowOffset` 修正偏移量计算 `targetScrollRow = Math.Max(1, activeCell.Row + scrollOffset)`，实现了点击超链接后带修正值自动将目标行滚动为页面第一行。
...
- **最新构建状态**：`ExcelAddInDemo.dll` 编译完全通过。完成顶部汇总行插入的优化：
  1. **完全移除样式复制**：根据指示，取消所有的 `Copy` 与 `PasteSpecial(xlPasteFormats)` 样式复制，完全依赖工作表原生的单元格美观样式。
  2. **直接填写数据**：纯代码直接在 `insertRow` 强类型填写 B 列名称、E 列“台”、F 列数量 1、G 列/H 列联动公式及 A 列序号公式。彻底断绝外部元数据。
