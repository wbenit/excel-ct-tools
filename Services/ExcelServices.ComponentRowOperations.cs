    ...
    public static partial class ExcelServices
    {
        ...
        private static void ExtractActiveComponentRowToClipboard(bool isCutMode)
        {
            try
            {
                ...
                int row = Convert.ToInt32(activeCell.Row);
                ...
                // 解析定位当前活动行所属的箱柜信息
                var cabInfo = FindCabinetByRow(sheet, row);
                if (cabInfo == null || !cabInfo.Value.IsComponentRow)
                {
                    // 若未处于合法元器件区间内则弹出警示
                    System.Windows.Forms.MessageBox.Show(
                        "请在箱柜的有效元器件数据行中执行此操作！\n(不能在箱柜信息行、表头行、小计行或计费区域操作)",
                        "提示",
                ...
                // 规则 7: 采用二维数组一次性读取整行数据至内存
                dynamic readRange = sheet.Range[sheet.Cells[row, 1], sheet.Cells[row, colCount]];
                ...
                {
                    // 检查 H 列 (总价) 是否包含公式
                    string formulaH = Convert.ToString(sheet.Cells[row, 8].Formula) ?? "";
                    if (formulaH.StartsWith("=")) formulas[8] = formulaH;

                    // 检查 K 列 (成本总价) 是否包含公式
                    string formulaK = Convert.ToString(sheet.Cells[row, 11].Formula) ?? "";
                    if (formulaK.StartsWith("=")) formulas[11] = formulaK;
                }
                catch { }

                // 提取 CadHandle 与 HandleB
                string handleVal = "";
                string handleBVal = "";
                ...
                var exchangeDto = new ComponentRowExchangeDto
                {
                    ...
                    SourceCabinetK = cabInfo.Value.CabinetK,
                    SourceRowIndex = row,
                ...
                app.StatusBar = $"[{actionText}元件] 行 {row}: {nameVal} {modelVal}";
            }
            catch (Exception ex)
            {
                // 记录异常日志
        ...
        public static void InsertCopiedOrCutComponentRow()
        {
            try
            {
                ...
                // 定位当前活动行所属的目标箱柜
                var targetCab = FindCabinetByRow(targetSheet, targetRow);
                if (targetCab == null)
                {
                    System.Windows.Forms.MessageBox.Show(
                ...
                if (targetRow >= targetCab.Value.SubsumRow)
                {
                    targetRow = targetCab.Value.SubsumRow;
                }
                else if (targetRow < targetCab.Value.CompStartRow)
                {
                    // 若选中表头行或信息行，则插入为该箱柜的第一行元件
                    targetRow = targetCab.Value.CompStartRow;
                }
                ...
                bool isCrossCabinet = !string.Equals(clipData.SourceSheetName, targetSheetName, StringComparison.OrdinalIgnoreCase)
                                      || clipData.SourceCabinetK != targetCab.Value.CabinetK;
                ...
                if (isCrossCabinet)
                {
                    // 跨箱柜插入，将第 30 列 (AD 列) 的 CadHandle 强制置空
                    int hIdx = CadHandleColIndex; // 1-based 下标
                    if (hIdx <= valuesToWrite.GetLength(1))
                    {
                        valuesToWrite[1, hIdx] = null;
                    }

                    // 同步将第 31 列 (AE 列) 的辅助 Handle 强制置空
                    int hbIdx = CadHandleBColIndex;
                    if (hbIdx <= valuesToWrite.GetLength(1))
                    {
                        valuesToWrite[1, hbIdx] = null;
                    }
                }
                ...
                try
                {
                    // 在目标行执行物理整行下推插入 (Shift Down)
                    targetSheet.Rows[targetRow].Insert(XlInsertShiftDirection.xlShiftDown);

                    // 写入整行数据矩阵 (单次 COM 批量写入)
                    int writeCols = valuesToWrite.GetLength(1);
                    dynamic insertRange = targetSheet.Range[targetSheet.Cells[targetRow, 1], targetSheet.Cells[targetRow, writeCols]];
                    insertRange.Value2 = valuesToWrite;

                    // 恢复自适应标准计算公式 (F 列数量 * G 列单价)
                    targetSheet.Cells[targetRow, 8].Formula = $"=F{targetRow}*G{targetRow}";
                    // 恢复成本总价公式 (F 列数量 * J 列成本单价)
                    targetSheet.Cells[targetRow, 11].Formula = $"=F{targetRow}*J{targetRow}";
                    ...
                    if (clipData.IsCutMode)
                    {
                        ...
                        if (string.Equals(clipData.SourceSheetName, targetSheetName, StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            // 物理整行上移删除原剪切行
                            targetSheet.Rows[actualSourceRow].Delete(XlDeleteShiftDirection.xlShiftUp);
                        }
                        else
                        {
                            // 跨工作表剪切，定位来源工作表并物理删除原行
                            try
                            {
                                dynamic srcSheet = targetSheet.Parent.Worksheets[clipData.SourceSheetName];
                                if (srcSheet != null)
                                {
                                    srcSheet.Rows[clipData.SourceRowIndex].Delete(XlDeleteShiftDirection.xlShiftUp);
                                    // 刷新源工作表的序号与小计公式
                    ...
                    // 在状态栏反馈插入成功
                    app.StatusBar = $"[插入元件成功] 行 {targetRow}: {clipData.ComponentName} (跨柜清空Handle: {isCrossCabinet})";
                }
                finally
                {
                    // 恢复自动计算与屏幕刷新
        ...
        public static void DeleteComponentRow()
        {
            try
            {
                ...
                {
                    System.Windows.Forms.MessageBox.Show(
                        "请在箱柜的有效元器件数据行中执行【删除元件】！\n(严禁删除箱柜信息行、表头行、小计行或计费区域)",
                        "提示",
                        System.Windows.Forms.MessageBoxButtons.OK,
                        System.Windows.Forms.MessageBoxIcon.Information);
                    return;
                ...
                {
                    // 检测当前箱柜内现有元器件的总行数
                    int compRowCount = cabInfo.Value.CompEndRow - cabInfo.Value.CompStartRow + 1;

                    // 若箱柜内仅剩最后 1 行元器件
                    if (compRowCount <= 1)
                    {
                        // 规则 6: 保持箱柜基本骨架，清空该行元器件数据与公式 (保留空行)
                        int colCount = Math.Max(DefaultComponentColumnCount, GetSheetColumnCount(sheet));
                        object[,] blankRow = new object[1, colCount];
                        sheet.Range[sheet.Cells[row, 1], sheet.Cells[row, colCount]].Value2 = blankRow;

                        // 保留 A 列序号为 1
                        sheet.Cells[row, 1].Value2 = 1;
                    }
                    else
                    {
                        // 物理删除整行 (Shift Up)
                        sheet.Rows[row].Delete(XlDeleteShiftDirection.xlShiftUp);
                    }
        ...
        /// 箱柜行属性分析辅助模型
        /// </summary>
        private struct CabinetRowContext
        {
            public int CabinetK;
            public int DetRow;
            public int CompStartRow;
            public int CompEndRow;
            public int SubsumRow;
            public int TolsumRow;
            public bool IsComponentRow;
        }
